...
#include "jdDegree.h"
  ...
  jlQuaternion::jlQuaternion(const Radian& _x, const Radian& _y, const Radian& _z) {
    float fSinPitch = Math::sin(_x.valueRadians() * 0.5f);
    float fCosPitch = Math::sin(_x.valueRadians() * 0.5f);
  ...
  jlQuaternion::jlQuaternion(const Degree& _x, const Degree& _y, const Degree& _z) {
    //float fSinPitch = Math::sin(_x.valueRadians() * 0.5f);
  ...
  jlQuaternion::jlQuaternion(JDVector3 xyz, const float& w) : x(xyz.x), y(xyz.y), z(xyz.z), w(w){
  }
  ...
  jlQuaternion&
  jlQuaternion::rotate(Degree _x, Degree _y, Degree _z) {
  ...
  jlQuaternion::getEuler() {
    ...
    //euler.y = Math::asin(-2 * (x * z - w * y));
    //euler.z = Math::atan((2) * (x * z + w * y), 1 - (2 * (x * x + y * y)));
    //euler.z *= Math::RAD2DEG;
  ...
  JDMatrix4 jlQuaternion::getMatrix() const
  {
    JDMatrix4 mat;
